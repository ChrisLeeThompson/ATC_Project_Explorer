...
file merges data from three sources into a single structure:
...
def build_consolidated_metadata(
    ...
    three parsed data sources.

    Sites are matched by name across all three sources. If a
    site exists in the project data but not in the statistics
    or directory scan, the corresponding section is populated
    with empty defaults.
    ...
    :param project_root_path: Absolute path to the ATC project root
        ...
        resolved when loading from a saved JSON file.
