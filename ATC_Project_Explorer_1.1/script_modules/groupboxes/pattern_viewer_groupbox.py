...
class PatternCanvasWidget(QWidget):
        ...
        for draw_pass in (0, 1):
            for i, entry in enumerate(self._entries):
                ...
                if draw_pass == 1 and not is_selected:
                    continue
                self._draw_entry(
                    painter, entry, cx, cy, scale,
                    is_selected,
                )

        # Draw lamella (always on top)
        lamella_selected = (
            self._selected_index >= 0
            and self._selected_index < len(self._entries)
            and self._entries[self._selected_index].get(
                "is_lamella"
            )
        )
        self._draw_lamella(
            painter, cx, cy, scale, lamella_selected
        )
    ...
    def _compute_extents(self) -> tuple[float, float]:
        ...
        # Use the same effective lamella height as _draw_lamella
        effective_thickness = max(self._final_thickness_um, 0.2)
        max_h = effective_thickness
        lamella_half_w = self._lamella_width_um / 2
        lamella_half_t = effective_thickness / 2
    ...
    def _draw_lamella(
        ...
        # Use a minimum visible height for the lamella
        h_um = max(self._final_thickness_um, 0.2)
        h_px = h_um * scale
        h_px = max(h_px, 4)  # minimum 4px visible
        ...
        painter.drawRect(
            int(cx - w_px / 2), int(cy - h_px / 2),
            int(w_px), int(h_px),
        )

    def _draw_entry(
        self, painter: QPainter, entry: dict,
        cx: float, cy: float, scale: float,
        is_selected: bool,
    ) -> None:
        ...
        if _is_stress_relief(name):
            self._draw_stress_relief(
                painter, entry, cx, cy, scale, is_selected
            )
        else:
            self._draw_regular(
                painter, entry, cx, cy, scale, is_selected
    ...
    def _draw_regular(
        ...
        # Use the same effective lamella half-height as _draw_lamella
        # so patterns align with the visual lamella edges (which may
        # be enlarged by the minimum-height clamp).
        lamella_h_um = max(self._final_thickness_um, 0.2)
        lamella_h_px = max(lamella_h_um * scale, 4)
        lamella_half_px = lamella_h_px / 2
