...
class ATCDirectoryParser:
    ...
    def parse(
        ...
        :param cancel_check: Optional callable polled once per site
            directory (before its subtree is walked).  When it returns
            True, parsing stops immediately by raising
            :class:`OperationCancelled`, bounding cancellation latency
            to roughly one site's work.
