...
# PIL image mode -> (channel count, numpy-style dtype label).  Used to
...
def extract_image_metadata(image_path: Path) -> dict:
    ...
    # Only the first 8 bytes are needed, so read those rather than
    # the whole file.
    try:
...
def extract_xml_metadata(
    ...
    *,
    ...
    :param image_path: Absolute path to the image file.
    :param raw_bytes: Optional already-read file bytes.  When
        provided (non-TIF path), the file is not re-read from disk —
        callers that have already read the bytes (e.g. for a marker
        pre-scan) can pass them to avoid a second full read.
    ...
    embedded = _extract_embedded_xml_metadata(
...
def _get_basic_info(image_path: Path) -> dict:
    ...
    # Image dimensions.  Prefer PIL's lazy open, which reads only the
    # file header (no pixel decode) and reports the true bit depth via
    # its mode.  Fall back to a matplotlib decode only when PIL is
    # unavailable (Pillow is a recommended, not required, dependency).
    dims_found = False
    try:
        ...
        with Image.open(image_path) as pil_img:
            ...
            ch_dtype = _PIL_MODE_INFO.get(pil_img.mode)
            if ch_dtype is not None:
                channels, dtype = ch_dtype
                if channels > 1:
                    info["Channels"] = channels
                info["DataType"] = dtype
            else:
                # Unknown mode — record it so bit depth isn't lost
                info["DataType"] = pil_img.mode
        dims_found = True
    except ImportError:
        logger.debug(
            "PIL unavailable — using matplotlib to read dimensions"
        )
    except Exception:
        logger.debug(
            f"PIL could not read dimensions: {image_path.name}",
            exc_info=True,
        )
...
def _extract_embedded_xml_metadata(
    ...
    *,
    ...
    :param image_path: Path to the image file.
    :param raw_bytes: Optional already-read file bytes.  When
        provided, the file is not re-read from disk.
