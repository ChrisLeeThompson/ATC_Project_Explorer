...
def _rect_with_um(
    ...
    """Return a copy of *rect* augmented with numeric ``width_um`` and
    ``height_um`` (µm) fields for the canvas to draw from directly.

    This decouples canvas geometry from the display-formatted rect
    ...
    :param width_um: Optional pre-computed width in µm (e.g. a
        front/rear per-side width); when *None*, derived from
        ``rect["Width"]``.
    :param height_um: Optional pre-computed height in µm; when *None*,
        derived from ``rect["Height"]``.
    :return: A new rect dict with ``width_um`` and ``height_um`` set.
    """
    w = (
        width_um if width_um is not None
        else to_micrometres(rect.get("Width", "0"))
    )
    h = (
        height_um if height_um is not None
        else to_micrometres(rect.get("Height", "0"))
    )
    return {**rect, "width_um": w, "height_um": h}
...
def _build_display_entries(site_data: dict) -> list[dict]:
    ...
    for pd in pattern_data:
        ...
        # Numeric µm for the canvas — computed from the raw value so
        # geometry is not tied to the rounded display string above.
        offset_from_lamella_um = (
            to_micrometres(offset_raw) if offset_raw else 0.0
        )

        # Stress relief uses TrenchOffset for canvas positioning
        trench_offset_raw = recipe_act.get("TrenchOffset")
        trench_offset = _format_value(trench_offset_raw)
        trench_offset_um = (
            to_micrometres(trench_offset_raw)
            if trench_offset_raw else 0.0
        )
        ...
        common = {
            ...
            "offset_from_lamella_um": offset_from_lamella_um,
            "trench_offset": trench_offset,
            "trench_offset_um": trench_offset_um,
        ...
        if has_nonzero_overlaps:
            ...
            if front_height_raw is not None:
                ...
                front_h_um = to_micrometres(front_height_raw)
                front_entry["rectangles"] = [
                    _rect_with_um(r, front_w_um, front_h_um)
                    for r in rects
                ]
            else:
                front_entry["height"] = rect_height
                front_entry["rectangles"] = [
                    _rect_with_um(r, front_w_um)
                    for r in rects
                ]
            ...
            rear_entry["width"] = f"{round(rear_w_um, 2)} µm"
            if rear_height_raw is not None:
                rear_entry["height"] = _format_um_rounded(
                    rear_height_raw
                )
                rear_h_um = to_micrometres(rear_height_raw)
                rear_entry["rectangles"] = [
                    _rect_with_um(r, rear_w_um, rear_h_um)
                    for r in rects
                ]
            else:
                rear_entry["height"] = rect_height
                rear_entry["rectangles"] = [
                    _rect_with_um(r, rear_w_um)
                    for r in rects
                ]
...
class PatternCanvasWidget(QWidget):
    ...
    def wheelEvent(self, event) -> None:
        ...
        if delta > 0:
            # Zoom in (clamp at 50× to match the image canvas)
            factor = zoom_factor
            if self._zoom * factor > 50.0:
                return
        else:
    ...
    def mouseMoveEvent(self, event) -> None:
        ...
        if event.buttons() & Qt.MouseButton.LeftButton:
            ...
            # Only activate pan when there is something drawn; an
            # empty canvas should not show the closed-hand cursor
            # or pan a phantom view.
            if not self._drag_active and self._entries:
    ...
    def _compute_extents(self) -> tuple[float, float]:
        ...
        for entry in self._entries:
            ...
            if _is_stress_relief(name):
                ...
                trench_off = entry.get("trench_offset_um", 0.0)
                for rect in entry.get("rectangles", []):
                    rw = rect.get("width_um", 0.0)
                    rh = rect.get("height_um", 0.0)
                    total_w = (
                ...
                offset = entry.get("offset_from_lamella_um", 0.0)
                for rect in entry.get("rectangles", []):
                    rw = rect.get("width_um", 0.0)
                    rh = rect.get("height_um", 0.0)
                    if rw > max_w:
    ...
    def _draw_regular(
        ...
        offset_um = entry.get("offset_from_lamella_um", 0.0)
        ...
        for rect in entry.get("rectangles", []):
            w_um = rect.get("width_um", 0.0)
            h_um = rect.get("height_um", 0.0)
            if w_um <= 0 or h_um <= 0:
                continue
    ...
    def _draw_stress_relief(
        ...
        """
        color = QColor(entry.get("color", _DEFAULT_PATTERN_COLOR))
        trench_offset_um = entry.get("trench_offset_um", 0.0)
        lamella_half_w = self._lamella_width_um / 2
