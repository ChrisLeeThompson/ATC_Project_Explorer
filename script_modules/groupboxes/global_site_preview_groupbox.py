...
class _SitePreviewCard(QGroupBox):
    ...
    def populate(
        ...
        """Decode and display the two preview images (on-demand path).

        Used when no preloaded media is available; decodes the images
        from disk on the calling (GUI) thread.

        :param site_name: Display name shown as the groupbox title.
        :param left_path: Absolute path to the electron / evaluation
            image, or *None*.
        :param right_path: Absolute path to the polishing image, or
            *None*.
        """
        thumb_max = (
        ...
        left_pixmap = (
            load_preview_pixmap(left_path, max_dim=thumb_max)
            if left_path is not None else None
        )
        right_pixmap = (
            load_preview_pixmap(right_path, max_dim=thumb_max)
            if right_path is not None else None
        )
        self._set_previews(
            site_name,
            left_pixmap, left_path.name if left_path else "",
            right_pixmap, right_path.name if right_path else "",
        )

    def populate_from_qimages(
        self,
        site_name: str,
        left_qimage,
        left_tooltip: str,
        ...
        """Display preloaded preview images (preloaded-media path).

        The ``QImage`` thumbnails were decoded off the GUI thread by
        ``preview_media_loader``; they are wrapped in ``QPixmap`` here,
        which must happen on the GUI thread.

        :param site_name: Display name shown as the groupbox title.
        :param left_qimage: Preloaded electron/evaluation QImage, or
            *None*.
        :param left_tooltip: Tooltip (filename) for the left image.
        :param right_qimage: Preloaded polishing QImage, or *None*.
        :param right_tooltip: Tooltip (filename) for the right image.
        """
        left_pixmap = (
            QPixmap.fromImage(left_qimage)
            if left_qimage is not None and not left_qimage.isNull()
            else None
        )
        right_pixmap = (
            QPixmap.fromImage(right_qimage)
            if right_qimage is not None and not right_qimage.isNull()
            else None
        )
        self._set_previews(
            site_name,
            left_pixmap, left_tooltip,
            right_pixmap, right_tooltip,
        )
    ...
    def _set_previews(
        ...
        """Apply the two thumbnails, the title, and clickable state.
        ...
        :param left_tooltip: Tooltip (filename) for the left image.
        :param right_pixmap: Right thumbnail QPixmap, or *None*.
        :param right_tooltip: Tooltip (filename) for the right image.
        """
        ...
        self._left_label.set_preview(
            left_pixmap, tooltip=left_tooltip
        )
        self._right_label.set_preview(
            right_pixmap, tooltip=right_tooltip
        )

        # Enable click interaction when at least one image path existed
        self._clickable = bool(left_tooltip or right_tooltip)
...
class GlobalSitePreviewGroupBox(QGroupBox):
    ...
    def set_preloaded_media(self, media: dict | None) -> None:
        ...
        When set before :meth:`populate`, each card is filled from the
        bundle's already-decoded ``QImage`` thumbnails instead of
        decoding on the GUI thread.  Sites absent from the bundle fall
        back to on-demand loading.
    ...
    def populate(self, metadata: dict) -> None:
        ...
        Existing cards are removed and rebuilt from scratch.  Cards
        use preloaded media when available, otherwise decode their
        ...
        for site_data in sites:
            ...
            site_media = (
                self._preloaded_media.get(site_name)
                if self._preloaded_media else None
            )
            if site_media is not None:
                card.populate_from_qimages(
                    site_name,
                    site_media.get("left_qimage"),
                    site_media.get("left_tooltip", ""),
                    site_media.get("right_qimage"),
                    site_media.get("right_tooltip", ""),
                )
            else:
